...
class TAPDriver:
  ...
  def write_IR(self,dr_addr,ir_exp=None):
    ...
    self.shift(dr_addr_value, TAPDriver.IR_bits,expect=ir_exp_value) #Shift in the data and exit1
...
def test_tap():
    ...
    tap_v = m.DefineFromVerilogFile("tests/build/tap_verilog.v",
                                type_map={"TCK": m.In(m.Clock)})[0]
