...
Fifo = DefineCircuit("Fifo", "empty", Out(Bit), "rdata", Out(Bits(4)), "full", Out(Bit), "wdata", In(Bits(4)), "wen", In(Bit), "ren", In(Bit), *ClockInterface(has_ce=False))
...
for i in range(len(memory)):
    memory_next_0_tmp[i] = Mux(2, len(mux([x.O for x in memory], i)))(mux([
        x.O for x in memory], i), wdata, __silica_decoder_8[i])
...
__silica_decoder_8 = decoder(waddr.O)
for i in range(len(memory)):
    memory_next_1_tmp[i] = Mux(2, len(mux([x.O for x in memory], i)))(mux([
        x.O for x in memory], i), wdata, __silica_decoder_8[i])
...
__silica_decoder_8 = decoder(waddr.O)
for i in range(len(memory)):
    memory_next_4_tmp[i] = Mux(2, len(mux([x.O for x in memory], i)))(mux([
        x.O for x in memory], i), wdata, __silica_decoder_8[i])
...
__silica_decoder_8 = decoder(waddr.O)
for i in range(len(memory)):
    memory_next_5_tmp[i] = Mux(2, len(mux([x.O for x in memory], i)))(mux([
        x.O for x in memory], i), wdata, __silica_decoder_8[i])
